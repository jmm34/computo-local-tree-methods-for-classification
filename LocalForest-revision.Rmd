...
Our proposal to introduce weights in the splitting criterion (see Section \@ref(sec:localSplittingRules): LazyDT and kernel approaches) is problematic. 
...
The CSRF of @xu:etal:2016 (see Section \@ref(subsec:weightingIndividuals)), the nearest neighbor weights (Section \@ref(subsec:nearest-neighbours)) and the local weighting 
of covariates (Section \@ref(sec:weightingCovariates)) strategies can give good performance but depend on tuning parameters.
