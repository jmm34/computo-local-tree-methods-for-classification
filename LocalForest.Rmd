...
The approximate Bayesian computation (ABC, @tavare:etal:1997; @pritchard:etal:1999) is a statistical method developed for frameworks where the likelihood is intractable. It relies on simulations according to Bayesian hierarchical models to generate pseudo-data. These artificial data are then compared to the test/observed one. To this effec, the most basic algorithm is based on nearest neighbors (NN). Recently, @breiman:2001's machine learning algorithm of random forests (RF) proved to bring a meaningful improvement to the ABC paradigm in both a context of model choice [@pudlo:etal:2016] and parameter inference [@raynal:etal:2019]. 
...
## Unidimensional kernel approach {#subsec:localKernel}
...
$$
    ...
    \Rightarrow \quad  \;\;\; \widetilde{p}_k = \frac{\sum_{i:x\idxi\in t} \mathbf{1}\{ y\idxi=k \} \Khj{i}}{\sum_{\ell:x^{(\ell)}\in t} \Khj{\ell}},
$$
...
\begin{equation}
   \underbrace{  
   \sum_{k=1}^K  \widetilde{p}_k (1-\widetilde{p}_k) }_{    \let\scriptstyle\textstyle\substack{\widetilde{I}(t)}}
- \Big(\frac{\widetilde{\#t_L}}{\widetilde{\#t}} \underbrace{ 
  \sum_{k=1}^K \widetilde{p}_{k,L} (1-\widetilde{p}_{k,L}) }_{    \let\scriptstyle\textstyle\substack{\widetilde{I}(t_L)}} 
   +\frac{\widetilde{\#t_R}}{\widetilde{\#t}} \underbrace{ 
    \sum_{k=1}^K \widetilde{p}_{k,R} (1-\widetilde{p}_{k,R}) }_{    \let\scriptstyle\textstyle\substack{\widetilde{I}(t_R)}} 
\Big)
   (\#eq:critlocalKernel)
\end{equation}
...
$$
The weighted proportions of individual at the node \@ref(eq:kernelProp) are transformed in the same way, resulting in a gain criterion analogous to \@ref(eq:critlocalKernel).
...
******
  ...
  Step 2.  For each training data $(y\idxi, x\idxi)$, count $c\idxi$ the number of times $x\idxi$ and $\xs$ ends in the same leaf
...
```{r gaussEq, out.width="50%", fig.cap="First Gaussian example: two first explanatory variables $X_1$ and $X_2$ ;    colors indicate the class labels (1-orange, 2-cyan, 3-purple, 4-green).", fig.align='center', echo=TRUE}
set.seed(1)
library(mvtnorm)
...
```{r gaussEq10, message=FALSE, warning=FALSE, results="asis", echo=TRUE}
library(xtable)
...
```{r gaussEqNoise10, message=FALSE, warning=FALSE, results="asis", echo=TRUE}
library(xtable)
resTOT<-read.table("Gaussian-Balanced-With-Noise/Example-Gaussian-Balanced-With-Noise-Res-10000-problem-csrf.txt",header=T)
resTOT<-100*resTOT
...
knitr::kable(TableOrdered,caption= "First Gaussian example: prediction error rate (percentage), with 10000 training data and 100 noise variables",booktabs=T,label= "gaussEqNoise10",row.names=NA)
...
```{r gaussDisp, out.width="50%", fig.cap="Second Gaussian example: two first explanatory variables $X_1$ and $X_2$ ;    colors indicate the classes (1-orange, 2-cyan, 3-purple, 4-green).", fig.align='center', echo=TRUE}
# Model probabilities (4 balanced classes)
pi0 <- 0.40
pi1 <- 0.40
pi2 <- 0.10
...
```{r gaussUnb10, message=FALSE, warning=FALSE, results="asis", echo=TRUE}
library(xtable)
resTOT<-read.table("Gaussian-Unbalanced-No-Noise/Example-Gaussian-Unbalanced-No-Noise-Res-10000-probelm-csrf.txt",header=T)
resTOT<-100*resTOT
...
knitr::kable(TableOrdered,caption= "Second Gaussian example: prediction error rate (percentage), with 10000 training data",booktabs=T,label= "gaussUnb10",row.names=NA)
...
```{r gaussUnbNoise10, message=FALSE, warning=FALSE, results="asis", echo=TRUE}
library(xtable)
resTOT<-read.table("Gaussian-Unbalanced-With-Noise/Example-Gaussian-Unbalanced-With-Noise-Res-10000-problem-csrf.txt",header=T)
resTOT<-100*resTOT
...
knitr::kable(TableOrdered,caption= "Second Gaussian example: prediction error rate (percentage), with 10000 training data and 100 noise variables",booktabs=T,label= "gaussUnbNoise10",row.names=NA)
...
```{r GenPopResults, message=FALSE, warning=FALSE, results="asis", echo=TRUE}
library(xtable)
resTOT<-read.table("Population-Genetics/Example-Population-Genetics-Res-problem-csrf.txt",header=T)
resTOT<-100*resTOT
...
The global computational time for the examples presented above represent several days of multiple cores usage and are therefore not directly proposed to the reader. All codes, data and session information are available at https://github.com/jmm34/computo-local-tree-methods-for-classification.

In this section, we reproduce the first Gaussian example presented above (without additional noise) with only 500 training data, 100 test data and 5 replicates, to illustrate the methods and produce similar tables to Tables \@ref(tab:gaussEq10) to \@ref(tab:GenPopResults).
...
```{r ToyEx, message=FALSE, warning=FALSE, eval=FALSE, results="asis", echo=TRUE}
...
knitr::kable(TableOrdered,caption= "Toy example: prediction error rate (percentage)",booktabs=T,label= "ToyEx",row.names=NA)
```
